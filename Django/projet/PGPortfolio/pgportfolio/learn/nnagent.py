...
class NNAgent:
    ...
    def decide_by_history(self, history, last_w):
        ...
        feed_dict = {self.__net.input_tensor: history, self.__net.previous_w: last_w[np.newaxis, 1:], self.__net.input_num: 1}
        res = np.squeeze(self.session.run(self.__net.output, feed_dict))
        return res
