...
class Trader:
    ...
    def __trade_body(self):
        # Ici se joue le backtest
        self._current_error_state = 'S000'
        starttime = time.time()
        omega = self._agent.decide_by_history(self.generate_history_matrix(), self._last_omega.copy()) # genere un nouveau omega
    ...
    def start_trading(self):
        try:
            if not self.__class__.__name__=="BackTest":
                ...
                time.sleep(wait) #enlevé le +2

                while self._steps < self._total_steps:
                    sleeptime = self.__trade_body()
                    # Un essai
                    # time.sleep(sleeptime)
                    print("time sleep:", sleeptime)
            else:
                while self._steps < self._total_steps:
                    print("Step:", self._steps, "/", self._total_steps)
                    self.__trade_body()
        finally:
            if self._agent_type=="nn":
                self._agent.recycle()
